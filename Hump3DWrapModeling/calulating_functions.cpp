	...
	for (int i = 1; i < dims[0] - 1; i++) {
		for (int k = 1; k < dims[2] - 1; k++)
			for (int j = 0; j < dims[1] - 2; j++) {
				...
				double xi1 = i * deltas[0] + xi1_min;
				double xi2 = k * deltas[2] + xi2_min;
				double sum = integrate(j, deltas, U, i, k, xi1, xi2, W);
				next[i][j][k] = sum;
			}
	}
...
double mu_derivative(double xi1, double xi2, int dim) {
	if (dim == 0)
		return -0.5 * 1 * xi1 * std::exp(-std::pow(xi1, 2.0) * alpha - std::pow(xi2, 2.0) * beta);
	else
		return -0.5 * 1 * xi2 * std::exp(-std::pow(xi1, 2.0) * alpha - std::pow(xi2, 2.0) * beta);
}
