...
double ***H(double ***U, SimulationParams params) {
    ...
    for (size_t i = 0; i < params.dims[0]; i++) {
        ...
        for (size_t k   = 0; k < params.dims[2]; k++) {
            ...
            next[i][0][k]                  =
                    -c * mu(xi1, xi2, params);                                // H| \theta = 0 == 0
...
double integrate(const size_t &j, double ***U, const size_t &i, const size_t &k, double xi1, double xi2, double ***W,
...
double v_func(int id1, double ***U, const size_t &i, const size_t &k, double xi1, double xi2, double ***W,
              SimulationParams params) {
...
double
central_derivative(double ***arr, int dim, int i, int j, int k, double delta, int size, SimulationParams params) {
    int ids[] = {i, j, k};
    ids[dim] -= 1;
